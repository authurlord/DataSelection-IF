...
DO_TRAIN_IF_SINGLE = False
DO_EVAL_IF_SINGLE = False
...
if DO_SELECT_ABLATION:
    ...
    if os.path.exists('Influence_single/{}/{}'.format(task,dataset)):
        ...
        for IF_method in sample_IF_single.keys():
            ...
            json.dump(select_IF_single_df.to_dict(orient='records'), open('train/{}/{}/train-select-w-IF-single-{}.json'.format(args.task,args.dataset,IF_method), 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
    ...
    json.dump(select_ppl_df.to_dict(orient='records'), open('train/{}/{}/train-select-w-ppl.json'.format(args.task,args.dataset), 'w', encoding='utf-8'), ensure_ascii=False, indent=4)

    json.dump(select_FL_df.to_dict(orient='records'), open('train/{}/{}/train-select-w-FL.json'.format(args.task,args.dataset), 'w', encoding='utf-8'), ensure_ascii=False, indent=4)

    json.dump(select_IF_df.to_dict(orient='records'), open('train/{}/{}/train-select-w-IF.json'.format(args.task,args.dataset), 'w', encoding='utf-8'), ensure_ascii=False, indent=4)
...
train_args['train_file_path'] = 'train/{}/{}/train-select.json'.format(args.task,args.dataset)
...
if DO_TRAIN_IF_SINGLE:
    ...
    for ablation_method in ['IF-single-iterative','IF-single-identity','IF-single-proposed']:
        train_args = load_yaml(template_yaml_path)
    ...
    for ablation_method in ['IF-single-iterative','IF-single-identity','IF-single-proposed']:
        ...
        try:
            ...
            if os.path.exists('eval_result/{}-{}-{}.npy'.format(model,task,dataset)):
                all_metrics = np.load('eval_result/{}-{}-{}.npy'.format(model,task,dataset))
            else:
                all_metrics = {}
            all_metrics[ablation_method] = metrics
        # all_metrics[ablation_method] = metrics
