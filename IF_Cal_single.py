...
for i in range(1,9,1): ## maximum of k process
    if os.path.exists('grad_single/{}/{}/tr_grad_{}.pkl'.format(task,dataset,i)): ## i-th gradient 
        ## tr_grad
        ...
        ## tr_grad_index
        tr_index_part = torch.load('grad_single/{}/{}/tr_grad_index_{}.pkl'.format(task,dataset,i))
        tr_index.extend(tr_index_part)
        
        ## val_grad
        val_grad_dict_part = torch.load('grad_single/{}/{}/val_grad_{}.pkl'.format(task,dataset,i))
...
os.makedirs('Influence_single/{}/{}'.format(args.task,args.dataset),exist_ok=True)


result_df.to_csv('Influence_single/{}/{}/result_df.csv'.format(args.task,args.dataset))

create_folder_for_file('Influence_single/{}/{}/result.pkl'.format(args.task,args.dataset))
torch.save(sorted_dict_all,'Influence_single/{}/{}/score.pkl'.format(args.task,args.dataset))
if args.save_all_layers:
    torch.save(result,'Influence_single/{}/{}/result.pkl'.format(args.task,args.dataset))
