...
def selec_data_from_corpus(
    ...
    while True:
        ...
        if candidate_budget == 'all':
            # 计算冗余度
            group_information_redundancy_state = pool.map(get_compression_ratio, 
                                                          [str(anchor_data + selected_data + [part]) for part in data_list])
            ...
            group_information_redundancy_state = pool.map(get_compression_ratio, 
                                                          [str(anchor_data + selected_data + [part]) for part in group_list])
            group_information_redundancy_state = torch.tensor(group_information_redundancy_state)
...
def ZIP_select(data_pool, save_path, budget, k1=10000, k2=200, k3=100, n_jobs=1):
    ...
    # 初始化数据冗余状态
    global_information_redundancy_state = pool.map(get_compression_ratio, 
                                                  [str(part) for _, part in data_pool.iterrows()])
    ...
    while len(final_selected_data) < budget:
        ...
        third_stage_index, third_stage_data = selec_data_from_corpus(
            [], second_stage_index, k3, 1, k3, pool, turn_print=False,
            data_pool=data_pool,
            global_information_redundancy_state=global_information_redundancy_state
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_jobs', type=int, default=64, help='用于计算压缩比的并行作业数')
    ...
    # 运行ZIP_select函数
    ZIP_select(df, args.save_path, args.budget, args.k1, args.k2, args.k3, args.n_jobs)
