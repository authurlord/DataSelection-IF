...
for index in range(len(expert_combination)): ## extract standalone loras
    ...
    task_list = list(result[result['labels']==expert_combination[index]]['task'].unique())
    ...
    ## Select File
        ...
        selection_list = list(result_df['output'].unique())
        selection_list = [s for s in selection_list if s!='']
        print(len(selection_list))
        ...
        result.to_csv('../train/router/test_guided_MoE.csv')
result.to_csv('../train/router/test_guided_MoE.csv')
