...
from prismatic.training.strategies.base_strategy import TrainingStrategy, TrainingStrategyLLM

# Initialize Overwatch =>> Wraps `logging.Logger`
overwatch = initialize_overwatch(__name__)

class FSDPStrategyLLM(TrainingStrategyLLM):
    def __init__(
        self,
        llm: PrismaticLLM,
        device_id: int,
    ...
    def save_checkpoint(
        ...
        assert isinstance(self.llm, FSDP), "FSDPStrategy.save_checkpoint assumes LLM is already wrapped in FSDP!"

        # Summon Full State Dictionary =>> Reconstitute from Shards
        with FSDP.state_dict_type(self.llm, self.fsdp_state_dict_type, self.fsdp_save_policy):
            full_llm_state_dict = self.llm.state_dict()
            model_state_dicts = {
            ...
            for key, param in full_llm_state_dict.items():
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        llm_fsdp_wrapping_policy = self.llm.get_fsdp_wrapping_policy()
        ...
        if self.enable_mixed_precision_training and self.mixed_precision_dtype == torch.bfloat16:
            ...
            self.llm.vision_backbone.to(dtype=self.llm.vision_backbone.half_precision_dtype)
        ...
        self.llm = FSDP(
            self.llm,
            auto_wrap_policy=llm_fsdp_wrapping_policy,
        ...
        if self.enable_gradient_checkpointing:
            ...
            apply_activation_checkpointing(self.llm, checkpoint_wrapper_fn=non_reentrant_wrapper, check_fn=check_fn)
    ...
    def clip_grad_norm(self) -> None:
        ...
        self.llm.clip_grad_norm_(max_norm=self.max_grad_norm)
