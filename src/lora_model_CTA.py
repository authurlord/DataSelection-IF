...
class LORAEngineDebertaMultiClass(object):
    ...
    #                 )
    #             metric_macro = metric.copy()
    #             eval_metric = metric.compute(average="micro")
    #             eval_metric_macro = metric_macro.compute(average="macro")
    #             print(f"Epoch {(epoch + 1)}:", eval_metric, eval_metric_macro)
    ...
    def train_LORA_model(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if self.valid_each_epoch:
                metric_micro = evaluate.load("../../evaluate-main/metrics/f1", "f1")
                metric_macro = evaluate.load("../../evaluate-main/metrics/f1", "f1")
                self.model.eval()
                for step, batch in enumerate(tqdm(self.eval_dataloader)):
                    batch.pop('id', None)
                    batch = batch.to(self.device)
                    ...
                    predictions = outputs.logits.argmax(dim=-1)
                    predictions, references = predictions, batch["labels"]
                    metric_micro.add_batch(
                        predictions=predictions,
                        references=references,
                    )
                    metric_macro.add_batch(
                        predictions=predictions,
                        references=references,
                    )
                # 直接使用同一个metric对象分别计算micro和macro的F1值，无需复制对象
                eval_metric_micro = metric_micro.compute(average="micro")
                eval_metric_macro = metric_macro.compute(average="macro")
                print(f"Epoch {(epoch + 1)}: Micro F1 = {eval_metric_micro}, Macro F1 = {eval_metric_macro}")
        ...
        eval_metric_macro = metric_macro.compute(average="macro")
        # print("Prediction Result on Test Data:", eval_metric)
        print(f"Prediction Result on Test Data: Micro F1 = {eval_metric_micro}, Macro F1 = {eval_metric_macro}")
        return tr_grad_dict_all,val_grad_dict_all
