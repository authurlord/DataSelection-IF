...
select_token_total = int(avg_token_count * 120)

select_command = 'CUDA_VISIBLE_DEVICES={} python -m data_tools.select_subset data/annotation-main/{}-{}/ data/subset-main-top100/{}-{}/ \
...
subset = datasets.concatenate_datasets([datasets.load_from_disk(ds) for ds in sorted(glob.glob("data/subset-main-top100/{}-{}/*".format(task,dataset)))])
...
train_file_select = train_file.iloc[index_list[:100]]

json.dump(train_file_select.to_dict(orient='records'), open(f'../train/{task}/{dataset}/train-select-w-QuRating-top100.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=4)

print(f'file dumps with size {len(train_file_select)}')
